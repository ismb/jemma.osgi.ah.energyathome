	...
	private class CachedData {
				...
				for (Iterator<ContentInstance> iterator = cachedResult.getResult().iterator(); iterator.hasNext();) {
					...
					if (addAll) {
						...
						if (cachedStartId - startId <= 1000) {
							...
							if (c.get(Calendar.HOUR_OF_DAY) == startIdHourOfDay && c.get(Calendar.DAY_OF_YEAR) == startIdDayOfYear && c.get(Calendar.YEAR) == startIdYear)
		...
		synchronized void update() {
			...
			synchronized (aeci) {
				...
				LOG.debug("Cache update for appliance " + applianceId + " - finished date/time initialization - elapsed time in millisec: "
			...
			LOG.debug("Cache update for appliance " + applianceId + " - energy and cost cache update finished - elapsed time in millisec: "
					+ (System.currentTimeMillis() - execTimeMillis));
			if (logCachedData) {
