...
public class ESPConfiguration {
	...
	public static ESPConfigParameters getConfigParameters() {
		...
		if (configProperties.getProperty(INITIAL_TIME_PROPERTY_NAME) != null) {
			try {
				...
				float peakProducedPower = (peakProducedPowerProperty == null || peakProducedPowerProperty.length() == 0) ? 0 : Float.parseFloat(peakProducedPowerProperty);
				;
	...
	public static void setConfigParameters(ESPConfigParameters configParameters) throws ESPException {
		if (configParameters == null) {
			...
			configProperties.setProperty(CONTRACTUAL_POWER_THRESHOLD_PROPERTY_NAME, Float.toString(configParameters.getContractualPowerThreshold()));
			configProperties.setProperty(PEAK_PRODUCED_POWER_PROPERTY_NAME, Float.toString(configParameters.getPeakProducedPower()));
			saveProperties();
		}
	}
