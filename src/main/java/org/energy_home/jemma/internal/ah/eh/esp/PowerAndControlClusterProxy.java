...
public class PowerAndControlClusterProxy extends ServiceClusterProxy implements PowerProfileClient, IPowerAndControlProxy {
	...
	public class ApplianceControlProxy extends ServiceClusterProxy implements ApplianceControlClient {
		...
		public void execSignalStateNotification(short ApplianceStatus, short RemoteEnableFlags, int ApplianceStatus2, IEndPointRequestContext context) throws ApplianceException,
			...
			LOG.debug(String.format("ApplianceControlClient[%s].execSignalStateNotification(ApplianceStatus=%s, RemoteEnableFlags=%s, ApplianceStatus2=%s)", applianceId,
					ApplianceStatus, RemoteEnableFlags, ApplianceStatus2));
	...
	public class ApplianceStatisticsProxy extends ServiceClusterProxy implements ApplianceStatisticsClient {
		...
		public void execLogNotification(long timestamp, long logID, long logLength, byte[] logPayload, IEndPointRequestContext context) throws ApplianceException,
			...
			LOG.debug(String.format("ApplianceStatisticsClient[%s].execLogNotification(Timestamp=%s, LogID=%s, LogLenght=%s, LogPayload=%s)", applianceId, timestamp, logID,
					logLength, Utils.asHex(logPayload)));
	...
	public GetPowerProfilePriceExtendedResponse execGetPowerProfilePriceExtended(short options, short powerProfileID, int powerProfileStartTime, IEndPointRequestContext context)
		...
		String applianceId = getApplianceId(context);
		LOG.debug(String.format("PowerProfileClient[%s].execGetPowerProfilePriceExtended(Options=%s, PowerProfileID=%s, PowerProfileStartTime=%s)", applianceId, options,
				powerProfileID, powerProfileStartTime));
	...
	public void execPowerProfileNotification(short totalProfileNum, short powerProfileID, PowerProfileTransferredPhase[] powerProfileTransferredPhases,
		...
		String applianceId = getApplianceId(context);
		LOG.debug(String.format("PowerProfileClient[%s].execPowerProfileNotification(TotalProfileNum=%s, PowerProfileID=%s, PowerProfileTransferredPhases=%s)", applianceId,
				totalProfileNum, powerProfileID, Arrays.toString(powerProfileTransferredPhases)));
	...
	public void execEnergyPhasesScheduleStateNotification(short powerProfileID, ScheduledPhase[] scheduledPhases, IEndPointRequestContext context) throws ApplianceException,
			ServiceClusterException {
		String applianceId = getApplianceId(context);
		LOG.debug(String.format("PowerProfileClient[%s].execEnergyPhasesScheduleStateNotification(powerProfileID=%s, scheduledPhases=%s)", applianceId, powerProfileID,
				Arrays.toString(scheduledPhases)));
	...
	public void execPowerProfileScheduleConstraintsNotification(short powerProfileID, int startAfter, int stopBefore, IEndPointRequestContext context) throws ApplianceException,
			ServiceClusterException {
		String applianceId = getApplianceId(context);
		LOG.debug(String.format("PowerProfileClient[%s].execPowerProfileScheduleConstraintsNotification: PowerProfileID=%s, StartAfter=%s, StopBefore=%s", applianceId,
	...
	public void notifyProposedEnergyPhasesSchedule(String applianceId, short powerProfileID, EnergyPhaseScheduleTime[] epst) {
		...
		try {
			LOG.debug(String.format("PowerProfileServer[%s].execEnergyPhasesScheduleNotification(powerProfileID=%s, SchedulePhase=%s)", applianceId, powerProfileID,
					Arrays.toString(sp)));
			DeviceProxy deviceProxy = proxy.getDeviceProxy(applianceId);
			PowerProfileServer pps = getRemotePowerProfileCluster(deviceProxy);
			IEndPointRequestContext context = getApplicationRequestContext(deviceProxy, false);
