...
public class ESPContainersDataUtils {
	...
	public static boolean checkDuration(Calendar c, long startTime, long duration, int resolution) {
		...
		boolean result = duration >= minExpectedDuration * (1 - DURATION_ERROR_TOLERANCE) && duration <= maxExpectedDuration * (1 + DURATION_ERROR_TOLERANCE);
		...
		if (!result && resolution == ESPService.HOUR_RESOLUTION) {
			...
			if (dstOffset != 0) {
				...
				result = duration >= minExpectedDuration * (1 - DURATION_ERROR_TOLERANCE) && duration <= maxExpectedDuration * (1 + DURATION_ERROR_TOLERANCE);
			}
		}
		return result;
	}
	...
	public static ContentInstanceItems getNormalizedItems(ContentInstanceItems items, Calendar c, long normalizedStartTime, long normalizedEndTime, int resolution)
		...
		if (resultSize > 0) {
			...
			for (Iterator<ContentInstance> iterator = itemList.iterator(); iterator.hasNext();) {
				...
				if (content != null && checkDuration(c, contentInstance.getId().longValue(), content.getDuration(), resolution))
					...
					LOG.debug("getNormalizedItems - Added null item,  invalid duration: value=" + content.getValue() + ", startTime=" + contentInstance.getId().longValue()
	...
	public static ContentInstanceItemsList getNormalizedItemsList(ContentInstanceItemsList itemsList, Calendar c, long normalizedStartTime, long normalizedEndTime, int resolution)
			throws M2MHapException {
		if (itemsList == null)
			return null;
