...
public class ApplianceInfo {
	...
	public EnergyCostInfo updateEnergyCost(long newTime, double newEnergy) {
		...
		if (newTime < lastNotificationTime) {
			...
			// String.format calls inside debug messages in this class ?? if
			// not, we should use normal logging messages to avoid function
			// calls - leaving as it is right now
		...
		try {
			...
			if (elapsedTime > 0 && elapsedTime < MAX_VALID_EPOC_THRESHOLD) {
				...
				LOG.warn(String.format("updateEnergyCost %s: received new time value %d , elapsed time %d > MAX_VALID_EPOC_THRESHOLD, resetting start time", applianceId, newTime,
						elapsedTime));
